...
Panel {
  ...
  function dnsCommand(provider) {
    var command = root.omarchyPath ? (root.omarchyPath + "/bin/network-dns") : "network-dns"
  ...
  Process {
    id: detailsProc
    command: [root.omarchyPath ? (root.omarchyPath + "/bin/network-status") : "network-status", "--verbose"]
  ...
  Process {
    id: bandProc
    command: [root.omarchyPath ? (root.omarchyPath + "/bin/network-band") : "network-band"]
    stdout: StdioCollector {
      waitForEnd: true
