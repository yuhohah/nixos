...
Panel {
  ...
  function toggleDisplay(name, enabled) {
    ...
    Quickshell.execDetached(["hyprctl", "keyword", "monitor", name + (enabled ? ",disable" : ",preferred,auto,auto")])
    refreshTimer.restart()
  }

  function setScale(scale) {
    var mon = root.focusedMonitor || "eDP-1"
    Quickshell.execDetached(["hyprctl", "keyword", "monitor", mon + ",preferred,auto," + scale])
    refreshTimer.restart()
  }
  ...
  function setTextSize(px) {
    Quickshell.execDetached([root.omarchyPath ? (root.omarchyPath + "/bin/display-text-size") : "display-text-size", String(px)])
  }
  ...
  Process {
    id: stateProc
    command: [root.omarchyPath ? (root.omarchyPath + "/bin/monitor-state") : "monitor-state"]
