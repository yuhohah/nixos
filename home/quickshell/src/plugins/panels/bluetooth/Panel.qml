...
Panel {
  ...
  function deviceCommand(action, address) {
    if (action === "connect") {
      return ["bash", "-c", "bluetoothctl trust " + Util.shellQuote(address) + " 2>/dev/null; bluetoothctl connect " + Util.shellQuote(address)]
    } else if (action === "disconnect") {
      return ["bash", "-c", "bluetoothctl disconnect " + Util.shellQuote(address)]
    } else if (action === "pair") {
      return ["bash", "-c", "bluetoothctl pairable on 2>/dev/null; bluetoothctl pair " + Util.shellQuote(address) + "; bluetoothctl trust " + Util.shellQuote(address) + " 2>/dev/null; bluetoothctl connect " + Util.shellQuote(address)]
    } else if (action === "forget" || action === "remove") {
      return ["bash", "-c", "bluetoothctl disconnect " + Util.shellQuote(address) + " 2>/dev/null; bluetoothctl untrust " + Util.shellQuote(address) + " 2>/dev/null; bluetoothctl remove " + Util.shellQuote(address)]
    }
  ...
  function toggleBluetooth() {
    if (!adapter) return
    if (adapter.enabled) {
      Quickshell.execDetached(["bash", "-c", "bluetoothctl power off; rfkill block bluetooth 2>/dev/null || true"])
    } else {
      Quickshell.execDetached(["bash", "-c", "rfkill unblock bluetooth 2>/dev/null || true; bluetoothctl power on"])
    }
  }
