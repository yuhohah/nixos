...
Item {
  ...
  function startPhase(nextPhase) {
    ...
    phase = nextPhase
    stderrText = ""
    speedTestProc.command = [root.omarchyPath ? (root.omarchyPath + "/bin/network-speedtest") : "network-speedtest", nextPhase]
  ...
  Process {
    id: statusProc
    command: [root.omarchyPath ? (root.omarchyPath + "/bin/network-status") : "network-status"]
